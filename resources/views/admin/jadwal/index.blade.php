                        ...
                        <tbody>
                            @foreach ($registrations as $registration)
                                <tr class="text-center bg-gray-800 border-b border-gray-700">
                                    ...
                                    <td class="py-2 px-4">
                                        {{ optional($registration->schedules->firstWhere('scheduleName', 'Test Tulis'))->scheduleDate ? \Carbon\Carbon::parse($registration->schedules->firstWhere('scheduleName', 'Test Tulis')->scheduleDate)->format('d M Y') : '' }}
                                    </td>
                                    <td class="py-2 px-4">
                                        {{ optional($registration->schedules->firstWhere('scheduleName', 'Wawancara Asisten'))->scheduleDate ? \Carbon\Carbon::parse($registration->schedules->firstWhere('scheduleName', 'Wawancara Asisten')->scheduleDate)->format('d M Y') : '' }}
                                    </td>
                                    <td class="py-2 px-4">
                                        {{ optional($registration->schedules->firstWhere('scheduleName', 'Wawancara Dosen'))->scheduleDate ? \Carbon\Carbon::parse($registration->schedules->firstWhere('scheduleName', 'Wawancara Dosen')->scheduleDate)->format('d M Y') : '' }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
